"""
-- This code is for 2-D Darcy equation in triangular domain with notch.
"""
import pandas as pd
import numpy as np
import torch
...
l2.to_excel('results/uwnoDarcyNotchL2.xlsx')
...
for value in range(y_test.shape[0]):
    if value % 29 == 1:
        ...
        plt.imshow(x_test[value, :, :, 0], cmap='nipy_spectral', extent=[0, 1, 0, 1], origin='lower',
        ...
        plt.imshow(np.abs(y_test[index, :, :] - pred[index, :, :].cpu()), cmap='jet', extent=[0, 1, 0, 1],
        ...
        index = index + 1
# plt.savefig( 'figures/EX_3_1.pdf')
plt.show()

""" For saving the trained model and prediction data """
torch.save(model, 'model/UWNO_darcy_notch')
scipy.io.savemat('results/uwno_results_darcy_notch.mat', mdict={'x_test': x_test.cpu().numpy(),
