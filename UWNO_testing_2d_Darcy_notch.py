...
model = torch.load('model/UWNO_darcy_notch')
# 遍历所有子模块并检查类型
for name, module in model.named_modules():
    if isinstance(module, nn.GELU):
        print(f"发现 GELU 激活函数: {name}")
    elif "Mish" in str(type(module)):  # Mish 非 PyTorch 官方模块，需自定义检测
        print(f"发现 Mish 激活函数: {name}")
print(model)
...
figure1.text(0.07, 0.74, 'B.C.: $\ u(x,y)|_{\partial \omega}$', rotation=90, color='k', fontsize=15)
plt.subplots_adjust(wspace=0.7)
index = 0
for value in range(y_test.shape[0]):
    if value % 24 == 2 and value != 2:
        print(value)
        plt.subplot(4, 4, index + 1)
        plt.title('$\ B.C.-{}$'.format(index + 1), color='k', fontsize=15)
        plt.imshow(x_test[value, :, :, 0], cmap='bwr', extent=[0, 1, 0, 1], origin='lower',
        ...
        yf = xf * (ymax / (xmax / 2))
        plt.fill_between(xf, yf, ymax, color=[1, 1, 1])
        ...
        plt.imshow(y_test[value, :, :], origin='lower', extent=[0, 1, 0, 1], interpolation='Gaussian',
                   cmap='bwr')
        ...
        plt.imshow(pred[value, :, :].cpu(), origin='lower', extent=[0, 1, 0, 1], interpolation='Gaussian',
                   cmap='bwr')
        plt.colorbar(fraction=0.045)
        plt.xlabel('$\ x$')
        plt.ylabel('$\ y$')
        ...
        plt.imshow(np.abs(y_test[index, :, :] - pred[index, :, :].cpu()), cmap='bwr', extent=[0, 1, 0, 1],
        ...
        index = index + 1
# plt.savefig('figures/EX_4.3_1.pdf', bbox_inches='tight')
plt.show()