...
model = torch.load('model/UWNO_burgers')
...
for i in range(y_test.shape[0]):
    if i % 20 == 1:
        plt.subplot(2, 1, 1)
        plt.plot(np.linspace(0, 1, 1024), x_test[i, :].numpy(),linewidth=2)
        plt.title('I.C.')
        ...
        plt.subplot(2, 1, 2)
        plt.plot(np.linspace(0, 1, 1024), y_test[i, :].numpy(),linewidth=2, label='Exact')
        plt.plot(np.linspace(0, 1, 1024), pred[i, :].cpu().numpy(), '-.',linewidth=2, label='Prediction')
        plt.title('Solution')
        ...
        plt.yticks()
        plt.margins(0)
# plt.savefig('figures/EX_1.1_1.pdf',bbox_inches='tight')
plt.show()
# error = y_test-torch.squeeze(pred.cpu())
# error = torch.mean(error,dim=0).unsqueeze(-1)
# error = error - torch.mean(error)
# fft_error = torch.fft.fftn(error)
# fft_error = torch.fft.fftshift(fft_error)
# error = torch.abs(fft_error).numpy() # 计算幅度
# # 计算频率轴
# freq = torch.fft.fftfreq(1024, 1.0/1024)
# # 调整频率轴
# freq = torch.fft.fftshift(freq).numpy()# 平移频率轴，使频率中心置于零点
# n = 512
# # 绘制频谱图
# plt.figure(figsize=(10, 6))
# plt.plot(freq[n:n+n//6], error[n:n+n//6])
