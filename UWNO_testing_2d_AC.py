...
class UNet_2d(nn.Module):
    def __init__(self, in_channels):
        ...
        self.up1 = UpSample(2*in_channels, in_channels)
...
model = torch.load('model/UWNO_allencahn')
print(model)
...
figure1.text(0.06, 0.17, '\n  Error \n $\ |u-\hat{u}|$', rotation=90, color='k', fontsize=15)
figure1.text(0.06, 0.36, '\n  Prediction \n $\ \hat{u}(x,y,10)$', rotation=90, color='k', fontsize=15)
figure1.text(0.06, 0.56, '\n     Exact \n $\ u(x,y,10)$', rotation=90, color='k', fontsize=15)
figure1.text(0.07, 0.77, '     I.C. \n$\ u_{0}(x,y)$', rotation=90, color='k', fontsize=15)
plt.subplots_adjust(wspace=0.7)
index = 0
for value in range(y_test.shape[0]):
    if value % 26 == 1:
        plt.subplot(4, 4, index + 1)
        plt.imshow(x_test[value, :, :, 0], cmap='bwr', extent=[0, 3, 0, 3], interpolation='Gaussian')
        plt.title('IC-{}'.format(index + 1), color='k', fontsize=15)
        plt.xlabel('$\ x$')
        plt.ylabel('$\ y$')
        plt.xticks()
        plt.yticks()
        ...
        plt.imshow(y_test[value, :, :], cmap='bwr', extent=[0, 3, 0, 3], interpolation='Gaussian')
        plt.xlabel('$\ x$')
        plt.ylabel('$\ y$')
        plt.xticks()
        plt.yticks()
        ...
        plt.imshow(pred[value, :, :], cmap='bwr', extent=[0, 3, 0, 3], interpolation='Gaussian')
        plt.xlabel('$\ x$')
        plt.ylabel('$\ y$')
        plt.xticks()
        plt.yticks()
        ...
        plt.imshow(np.abs(y_test[value, :, :] - pred[value, :, :]), cmap='bwr', extent=[0, 3, 0, 3],vmax=6e-3,
        ...
        plt.margins(0)
        index = index + 1
# plt.savefig( 'figures/EX_4.5_1.pdf', bbox_inches='tight')
plt.show()

image_tensor = torch.mean(pred-y_test,dim=0)
image_tensor = image_tensor - torch.mean(image_tensor)

fft_image = torch.fft.fftn(image_tensor, dim=(-2, -1))


fft_image = torch.fft.fftshift(fft_image, dim=(-2, -1))

freq_x = torch.fft.fftfreq(image_tensor.shape[-1])
freq_y = torch.fft.fftfreq(image_tensor.shape[-2])

freq_x = torch.fft.fftshift(freq_x)
freq_y = torch.fft.fftshift(freq_y)

magnitude_spectrum = torch.log(torch.abs(fft_image) + 1)  # 取对数并加1以便于可视化

plt.figure(figsize=(10, 6))
plt.pcolormesh(freq_x, freq_y, magnitude_spectrum,cmap='jet',vmax=0.03)  # 仅绘制第一个通道的频谱
plt.title('Spectrum of the Image (Low Frequency Centered)')
plt.xlabel('Frequency (Hz)')
plt.ylabel('Frequency (Hz)')
plt.colorbar(label='Magnitude (log scale)')
# plt.savefig('figures/UWNOAC.pdf',bbox_inches='tight')
plt.show()