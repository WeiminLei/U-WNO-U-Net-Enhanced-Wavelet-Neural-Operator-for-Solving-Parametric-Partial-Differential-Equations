...
PATH_Train = 'data/piececonst_r421_N1024_smooth1.mat'
...
model = torch.load('model/UWNO_darcy')
print(model)
model.eval()
...
figure1.text(0.07, 0.72, 'Coefficient: $\ a(x,y)$', rotation=90, color='k', fontsize=15)
plt.subplots_adjust(wspace=0.7)
index = 0
for value in range(y_test.shape[0]):
    if value % 24 == 1 and value!=1:
        print(value)
        plt.subplot(4, 4, index + 1)
        plt.imshow(x_test[value, :, :, 0], cmap='bwr', extent=[0, 1, 0, 1], interpolation='Gaussian')
        plt.title('$\ a(x,y)-{}$'.format(index + 1), color='k', fontsize=15)
        plt.xlabel('$\ x$')
        plt.ylabel('$\ y$')
        plt.xticks()
        plt.yticks()
        ...
        plt.imshow(np.abs(pred[value, :, :] - y_test[value, :, :]), cmap='bwr', extent=[0, 1, 0, 1], vmax=3e-4,
        ...
        index = index + 1
plt.show()
# plt.savefig('figures/EX_4.2_1.pdf', bbox_inches='tight')