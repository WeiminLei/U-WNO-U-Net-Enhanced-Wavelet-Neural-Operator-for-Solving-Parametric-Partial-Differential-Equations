...
l2.to_excel('results/uwnoAdvectionL2.xlsx')
# %%
""" Prediction """
pred = []
test_e = []
with torch.no_grad():
    ...
    for x, y in test_loader:
        ...
        pred.append(out)
        print("Batch-{}, Test-loss-{:0.6e}".format(index, test_l2 / batch_size))
        index += 1
...
plt.grid(True)
plt.margins(0)
# plt.savefig( 'figure/EX_4_6_1_1.png')
plt.show()

"""
For saving the trained model and prediction data
"""
torch.save(model, 'model/UWNO_advection_time_independent')
scipy.io.savemat('results/uwno_results_advection_time_independent.mat', mdict={'x_test': x_test.cpu().numpy(),
