"""
-- This code is for 2-D Allen-Cahn equation.
"""
import pandas as pd
import numpy as np
import torch
...
l2.to_excel('results/uwnoACL2.xlsx')
# %%
""" Prediction """
pred = []
test_e = []
...
for value in range(y_test.shape[0]):
    if value % 25 == 1:
        ...
        plt.imshow(np.abs(y_test[value, :, :] - pred[value, :, :]), cmap='jet', extent=[0, 3, 0, 3],
        ...
        plt.margins(0)
        index = index + 1
plt.savefig( 'figures/EX_5.1_1.pdf')
plt.show()
# %%
"""
For saving the trained model and prediction data
"""
torch.save(model, 'model/UWNO_allencahn')
scipy.io.savemat('results/uwno_results_allencahn.mat', mdict={'x_test': x_test.cpu().numpy(),
